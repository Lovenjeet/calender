...
© Copyright 2009-2013  LuxSoft - www.LuxSoft.eu
...
function retrieve($start,$end,$iFilter='',$xFilter='') {
	...
	INNER JOIN [db]employe emp ON emp.EMPLOYEID = e.employee_id
...
function processEvent($from, $till, $eStart, $eEnd, &$row) {
	...
	for($i=$sTs;$i<=$eTs;$i+=86400) { //increment 1 day
		...
		if (strpos($row['xda'], $curD) === false) { //no exceptions
			...
			$evt['employee_id'] = stripslashes($row['emp_id']);
			$evt['employee_name'] = stripslashes($row['emp_name']);
			$evtList[$curD][] = $evt;
		}
	}
}
