...
<meta name="description" content="LuxCal web calendar - a Issam product">
<meta name="keywords" content="Issam, LuxCal, LuxCal web calendar">
...
if ($privs > 0) { //view rights
	...
	if ($privs == 9) { //admin rights
		echo "<select title=\"{$xx['hdr_select_admin_functions']}\" name='views' onchange='jumpMenu(this)'>
		<option value='#'>{$xx['hdr_admin']}&nbsp;</option>
		<option value='index.php?cP=90'".($cP == "90" ? " selected='selected'>" : '>').$xx['hdr_settings']."</option>
		<option value='index.php?cP=91'".($cP == "91" ? " selected='selected'>" : '>').$xx['hdr_categories']."</option>
		<option value='index.php?cP=92'".($cP == "92" ? " selected='selected'>" : '>').$xx['hdr_users']."</option>
		<option value='index.php?cP=93'".($cP == "93" ? " selected='selected'>" : '>').$xx['hdr_database']."</option>
		<option value='index.php?cP=94'".($cP == "94" ? " selected='selected'>" : '>').$xx['hdr_import_ics']."</option>
		<option value='index.php?cP=95'".($cP == "95" ? " selected='selected'>" : '>').$xx['hdr_export_ics']."</option>
		<option value='index.php?cP=96'".($cP == "96" ? " selected='selected'>" : '>').$xx['hdr_import_csv']."</option>
		<option value='index.php?cP=0'>{$xx['hdr_calendar']}</option>
</select> \n";
	}
	if ($set['navTodoList']) {
	...
	echo "<button type='button' title=\"{$xx['hdr_search']}\" onclick=\"window.location.href='index.php?cP=21'\">".($set['navButText'] ? $xx['hdr_button_search'] : '&nbsp;&#916;&nbsp;')."</button>\n";
	...
	echo "<button type='button' title=\"{$xx['hdr_options_panel']}\" onclick=\"toggleIHtml(this,'{$xx['hdr_button_options']}','{$xx['done']}'); show('optPanel','optMenu')\">{$xx['hdr_button_options']}</button>\n";
	echo "<form class='inline' method='post' id='gotoD' name='gotoD' action='index.php'>
		<input style='width:62px;' type='text' name='newD' id='newD' value='".IDtoDD($_SESSION['cD'])."'>
		<button type='button' title=\"{$xx['hdr_select_date']}\" onclick=\"dPicker(0,'gotoD','newD');return false;\">&larr;</button>
	</form>\n";
...
if ($privs > 0) { //view rights
	if ($set['navUpcoList']) {
		...
		echo "<div id='upcoBar'>
			...
			<h5 class='barHead' onmousedown='dragme(this,event)' onmouseup='dragme(this)'>{$xx['hdr_upco_list']}</h5>
			<div class='sideList'>\n";
	...
	if ($set['navTodoList']) {
		...
		echo "<div id='taskBar'>
		<img class='floatR point' onclick=\"show('taskBar')\" src='images/close.png' alt='close'><h5 class='barHead' onmousedown='dragme(this,event)' onmouseup='dragme(this)'>{$xx['hdr_todo_list']}</h5>\n";
