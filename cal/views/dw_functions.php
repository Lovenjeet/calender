...
function showGrid($date) {
	...
	foreach ($sFill as $k => $v) { //1 min = 1px
		...
		foreach ($v as $kk => $stM) { //event start time in mins
			...
			if ($chBox) {
				$attrib = $mayCheck ? "class='chkBox floatL point' onclick=\"checkE({$evt['eid']},'{$date}');\" title=\"{$xx['vws_check_mark']}\"" : "class='chkBox floatL'";
				$chBox = "<span {$attrib}>".trim($chBox)."</span>";
			}
			if ($set['eventHBox']) {
				$popText = "<b>{$time} {$evt['tit']}</b>";
			...
			echo "<div class='event'>{$chBox}".(($set['details4All'] or $mayEdit) ? "<span class='point' onclick=\"editE({$evt['eid']},'{$date}');\"" : "<span class='arrow'")."{$popAttr}>{$stiPrefix}{$evt['tit']}</span></div></div>\n";
		}
	}
}
