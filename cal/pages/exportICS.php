...
function selectEvents() {
	...
	echo "<form action='index.php' method='post'>
		...
		<tr><td class='label'>{$ax['iex_file_name']}:</td><td><input type='text' name='fileName' value=\"{$fileName}\" maxlength='40' size='26'> .ics</td></tr>
	...
	if (isset($_POST['create']) and $msg == $ax['iex_file_created']) {
		$fName = ($fileName ? $fileName : str_replace(' ','_',$set['calendarTitle'])).'.ics';
		$rName = str_replace('.','-'.date("Ymd-Hi").'.',$fName);
		echo "&nbsp;&nbsp;&nbsp;&nbsp;<button class='noPrint' type='button' onclick=\"location.href='dloader.php?ftd={$fName}&amp;rName={$rName}'\">{$ax['iex_download_file']}</button>\n";
	}
...
function makeFile() {
	...
	foreach ($evtList as $evtListDate) {
		foreach ($evtListDate as $evt) {
			if (!in_array($evt['eid'], $eidDone)) { //event not yet processed
				$vDescription = str_replace($from,$to,htmlspecialchars_decode(trim($evt['des']),ENT_QUOTES));
	...
	//save to iCal file
	$fullfName = "./files/".($fileName ? $fileName : str_replace(' ','_',$set['calendarTitle'])).".ics";
