...
$cLabel = isset($_POST['cLabel']) ? $_POST['cLabel'] : "";
$cMark = isset($_POST['cMark']) ? $_POST['cMark'] : "&#x2713;";
...
function showCategories($edit) {
			...
			while ($row=mysql_fetch_assoc($rSet)) {
				...
				if (!$edit) {
					echo "<td>[<a href='index.php?editCat=edit&amp;cid={$row['category_id']}'>{$ax['cat_edit']}</a>]</td>
						<td>".(($row['category_id'] != 1) ? "[<a href='index.php?delExe=y&amp;cid={$row['category_id']}'>{$ax['cat_delete']}</a>]" : "")."</td>\n";
				}
				echo "</tr>\n";
			}
		}
	...
	echo "</div>\n";
	if (!$edit) { echo "<button class='noPrint' type='button' onclick=\"window.location.href='index.php?editCat=add'\">{$ax['cat_add_new']}</button>\n"; }
	echo "<br><br>\n";
}
...
function editCategory($editCat,$cid) {
	...
	echo "<table class='list'>
		...
		<tr><td>{$ax['cat_approve']}:</td><td><input type='checkbox' name='approve' value='1'".($approve ? " checked='checked'> " : ' > ')."</td></tr>
		<tr><td>{$ax['cat_public']}:</td><td><input type='checkbox' name='public' value='1'".($public ? " checked='checked'> " : ' > ')."</td></tr>
		<tr><td>{$ax['cat_check_mark']}:</td><td><input type='checkbox' name='chBox' value='1'".($chBox ? " checked='checked'" : '').">
