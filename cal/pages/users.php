...
function showUsers() {
	...
	if (!$rSet) echo "Database Error.";
		...
		echo "<tr><td><table class='list'>
			<tr><th>{$ax['usr_id']}</th><th>{$ax['usr_name']}</th><th>{$ax['usr_email']}</th><th>{$ax['usr_rights']}</th><th>{$ax['usr_language']}</th><th>{$ax['usr_login_0']}</th><th>{$ax['usr_login_1']}</th><th>{$ax['usr_login_cnt']}</th><th></th></tr>\n";
		while ($row = mysql_fetch_assoc($rSet)) {
			...
			echo "</td>
				...
				<td>[<a href='index.php?editUser=y&amp;uid={$row['user_id']}'>{$ax['usr_edit']}</a>]";
	...
	echo "</table>
		<button class='noPrint' type='button' onclick=\"window.location.href='index.php?editUser=y&amp;uid=add'\">{$ax['usr_add']}</button>\n";
}
...
function editUser($uid) {
	...
	if ($uid == $_SESSION['uid'] or $uid == 2) {
		...
		echo "<td><select name='userRights'>
			...
			<option value='4'".($userRights == 4 ? " selected='selected'" : '').">{$ax['usr_post_manager']}</option>
			<option value='9'".($userRights == 9 ? " selected='selected'" : '').">{$ax['usr_admin_functions']}</option>
		</select></td>\n";
	}
	echo "</tr>
		<tr><td>{$ax['usr_ui_language']}:</td><td><select name='uiLanguage'>\n";
		$files = scandir("lang/");
	...
	echo "</select></td></tr>
		<tr><td>{$ax['usr_background']}:</td><td><input type='text' id='bgrnd' name='bgrnd' value='{$bgrnd}' size='8' maxlength='10'><button class='noPrint' title=\"{$ax['usr_select_color']}\" onclick=\"cPicker('bgrnd','uname','b');return false;\">&larr;</button></td></tr>
		</table>\n</td></tr>
		</table>\n";
	if ($uid == "add") {
...
function addUser() { //add user account
	global $uid, $userRights, $editUser, $uiLanguage, $ax, $uname, $email, $bgrnd, $rxEmail;
	...
	do {
		...
		$password = md5($new_pw);
		$result = dbQuery("INSERT INTO [db]users VALUES (NULL,'".mysql_real_escape_string($uname)."','$password',DEFAULT,'".mysql_real_escape_string($email)."',$userRights,DEFAULT,DEFAULT,DEFAULT,'$uiLanguage','$bgrnd',DEFAULT)");
...
function updateUser() { //update user account
	...
	do {
		...
		$result = dbQuery("UPDATE [db]users SET user_name = '".mysql_real_escape_string($uname)."', email = '".mysql_real_escape_string($email)."', privs = $userRights, language = '$uiLanguage', color = '$bgrnd' WHERE user_id = $uid");
