...
if (isset($_SESSION['uid']) and $_SESSION['uid'] == 1) {
	...
	if (isset($_POST["reg"]) or isset($_POST["exereg"])) { //register
		...
		echo "<div class='legend'>&nbsp;{$ax['log_register']}&nbsp;</div><br><br>
			<form method='post' action='index.php?xP=20'>
		...
		echo "<div class='legend'>&nbsp;{$ax['log_log_in']}&nbsp;</div><br><br>
			...
			<span class='floatR'><input type='checkbox' id='cookie' name='cookie' value='bake' ".(isset($_COOKIE['luxcal']) ? " checked='checked'" : '')."><label for='cookie'> {$ax['log_remember_me']}</label></span>
			<br><br><hr>
