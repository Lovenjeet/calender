...
function displayEvents($errors) {
	...
	echo "<div class='centerBox'>
		...
		{$ax['iex_select_delete_ignore']}<br><br>\n";
//display event list
	...
	echo "<tr><th>{$ax['iex_delete']}</th><th>{$ax['iex_title']}</th><th>{$ax['iex_venue']}</th><th>{$ax['iex_category']}</th><th>{$ax['iex_date']}</th><th>{$ax['iex_end_date']}</th><th>{$ax['iex_start_time']}</th><th>{$ax['iex_end_time']}</th><th>{$ax['iex_description']}</th><th>{$ax['iex_repeat']}</th></tr>\n";
	for ($i = 0; $i < $nofEvents; $i++) {
		...
		echo "<tr>
			<td class='floatC'><input type='checkbox' name='delete[{$i}]' value='1'".(empty($_POST['delete'][$i]) ? "" : " checked='checked'").">
...
//		echo '<td>r_t= '.$_POST['r_t'][$i].' r_i= '.$_POST['r_i'][$i].' r_p= '.$_POST['r_p'][$i].'<br> r_m= '.$_POST['r_m'][$i].' r_u= '.$_POST['r_u'][$i]."</td>\n"; //for testing
	...
	echo "</table>
		...
		<button class='noPrint' type='button' onclick=\"window.location.href='index.php?cP=94'\">{$ax['back']}</button>
		</form>
		</div>\n";
}
	...
	for ($i = 0; $i < $nofEvents; $i++) {
		if (empty($_POST['delete'][$i]) and $_POST['r_t'][$i] >= 0) { //if delete not ticked and repeat-type is valid
			...
			if (!eventInDb($title,$sDate,$eDate,$sTime,$eTime)) { //add event to db
				$q = "INSERT INTO [db]events VALUES (NULL,DEFAULT,'$title','$descr',$catID,'$venue',{$_SESSION['uid']},DEFAULT,DEFAULT,DEFAULT,DEFAULT,'$sDate','$eDate',DEFAULT,'$sTime','$eTime',$r_t,$r_i,$r_p,$r_m,'$r_u',DEFAULT,DEFAULT,'".date("Y-m-d")."','".date("Y-m-d")."',DEFAULT)";
				$result = dbQuery($q);
...
if ($privs == 9) { //admin
	...
	if (!isset($_POST['uploadFile']) and !isset($_POST['addEvents']) or (isset($_POST['uploadFile']) and $msg)) {
		...
		echo '<button type="button" onclick="window.location.href=\'index.php?cP=94\'">'.$ax['back']."</button>\n";
	}
	echo "</div>\n</div>\n";
} else {
