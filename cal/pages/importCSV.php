...
function uploadFile() {
	...
	echo "<form action='index.php' method='post' enctype='multipart/form-data'>
		...
		<input type='radio' name='dFormat' value='d-m-y'".($dFormat == 'd-m-y' ? " checked='checked'" : '')."> {$ax['dd_mm_yyyy']}&nbsp;&nbsp;&nbsp;
		<input type='radio' name='dFormat' value='m-d-y'".($dFormat == 'm-d-y' ? " checked='checked'" : '')."> {$ax['mm_dd_yyyy']}&nbsp;&nbsp;&nbsp;
		<input type='radio' name='dFormat' value='y-m-d'".($dFormat == 'y-m-d' ? " checked='checked'" : '')."> {$ax['yyyy_mm_dd']}</td></tr>
		<tr><td class='label'>{$ax['iex_time_format']}:</td><td>
		<input type='radio' name='tFormat' value='h:ma'".($tFormat == 'h:ma' ? " checked='checked'" : '')."> {$ax['time_format_us']}&nbsp;&nbsp;&nbsp;
		<input type='radio' name='tFormat' value='h:m'".($tFormat == 'h:m' ? " checked='checked'" : '')."> {$ax['time_format_eu']}</td></tr>
		</table>
...
function displayEvents($errors) {
	...
	echo "<p class='error'>{$ax['iex_number_of_errors']}: {$errors} ({$ax['iex_bgnd_highlighted']})</p><br>
		...
		<br>{$ax['iex_select_birthday_delete']}<br><br>\n";
//display event list
	...
	echo "<table>
		<tr><th>{$ax['iex_delete']}</th><th>{$ax['iex_birthday']}</th><th>{$ax['iex_title']}</th><th>{$ax['iex_venue']}</th><th>{$ax['iex_category']}</th><th>{$ax['iex_date']}</th><th>{$ax['iex_end_date']}</th><th>{$ax['iex_start_time']}</th><th>{$ax['iex_end_time']}</th><th>{$ax['iex_description']}</th></tr>\n";
	for ($i = 0; $i < $nofEvents; $i++) {
		...
		echo "<tr>
			...
			<td><input class='floatC' type='text'{$sdc} size='10' name='sDate[]' value='{$_POST['sDate'][$i]}'></td>
			<td><input class='floatC' type='text'{$edc} size='10' name='eDate[]' value='{$_POST['eDate'][$i]}'></td>
	...
	echo "</table>
		...
		<button class='noPrint' type='button' onclick=\"window.location.href='index.php?cP=96'\">{$ax['back']}</button>
		</form>\n";
}
	...
	for ($i = 0; $i < $nofEvents; $i++) {
		if (empty($_POST['delete'][$i])) {
			...
			if (!eventInDb($title,$sDate,$eDate,$sTime,$eTime)) { //update events db
				$q = "INSERT INTO [db]events VALUES (NULL,DEFAULT,'$title','$descr',$catID,'$venue',{$_SESSION['uid']},DEFAULT,DEFAULT,DEFAULT,DEFAULT,'$sDate','$eDate',DEFAULT,'$sTime','$eTime',$rType,$rInterval,$rPeriod,$rMonth,DEFAULT,DEFAULT,DEFAULT,'".date("Y-m-d")."', '".date("Y-m-d")."',DEFAULT)";
				$result = dbQuery($q);
...
if ($privs == 9) { //admin
	...
	if (!isset($_POST['uploadFile']) and !isset($_POST['addEvents']) or (isset($_POST['uploadFile']) and $msg)) {
		...
		echo "<button type='button' onclick=\"window.location.href='index.php?cP=96'\">{$ax['back']}</button>\n";
	}
	echo "</div>\n</div>\n";
} else {
