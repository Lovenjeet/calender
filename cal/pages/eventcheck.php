...
if (!isset($_POST['checkExe'])) { //get event details
	$rSet = dbQuery("
		SELECT e.user_id,e.title,e.checked,c.color,c.background,c.chbox,c.chlabel,u.user_name
		FROM [db]events e
...
echo "<input type='checkbox' name='cbx' value='yes'".($chk ? " checked='checked'" : '')."> {$clb}&nbsp;\n";
echo "</td></tr>\n";
echo "</table>\n";
